...
mixin template DefaultMesh() {
    ...
    uint getMeshBufferCount() {
        return irr_IMesh_getMeshBufferCount(cast(irr_IMesh*)(ptr));
    }

    IMeshBuffer getMeshBuffer(uint nr) {
        auto meshbuffer = irr_IMesh_getMeshBuffer(cast(irr_IMesh*)(ptr), nr);
        return cast(IMeshBuffer)meshbuffer;
    }

    MeshBuffer getMeshBuffer(const ref Material material) {
        auto meshbuffer = irr_IMesh_getMeshBufferByMaterial(cast(irr_IMesh*)(ptr), material.ptr);
        return new MeshBuffer(meshbuffer);
    }

    aabbox3df getBoundingBox() {
        auto box = irr_IMesh_getBoundingBox(cast(irr_IMesh*)(ptr));
    ...
    void setBoundingBox(aabbox3df box) {
        ...
        irr_IMesh_setBoundingBox(cast(irr_IMesh*)(ptr), temp);
    }

    void setMaterialFlag(MaterialFlag flag, bool newvalue) {
        irr_IMesh_setMaterialFlag(cast(irr_IMesh*)(ptr), flag, newvalue);
    }

    void setHardwareMappingHint(HardwareMappingHint newMappingHint, HardwareBufferType buffer=HardwareBufferType.VertexAndIndex) {
        irr_IMesh_setHardwareMappingHint(cast(irr_IMesh*)(ptr), newMappingHint, buffer);
    }

    void setDirty(HardwareBufferType buffer=HardwareBufferType.VertexAndIndex) {
        irr_IMesh_setDirty(cast(irr_IMesh*)(ptr), buffer);
    }
}
